...
$adminID = 1;
$sql = "SELECT name, email, phone, birth_date, gender, adminImage FROM Admin WHERE adminID = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $adminID);
$stmt->execute();
$stmt->bind_result($name, $email, $phone, $dob, $gender, $adminImage);
$stmt->fetch();
$stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['name']) && isset($_POST['email'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if (!empty($_FILES['profileImage']['name'])) {
            ...
            } elseif (is_uploaded_file($_FILES['profileImage']['tmp_name'])) {
                if (move_uploaded_file($_FILES['profileImage']['tmp_name'], $targetFilePath)) {
                    $success = "The profile image has been uploaded successfully.";
                } else {
                    $error = "Sorry, there was an error uploading your file.";
                }
            } else {
                $error = "The file is not a valid uploaded file.";
            }
        }

        if (empty($error)) {
            $updateAdminSql = "UPDATE Admin SET name = ?, email = ?, phone = ?, birth_date = ?, gender = ?, adminImage = ? WHERE adminID = ?";
            $stmt = $conn->prepare($updateAdminSql);
            $stmt->bind_param("ssssssi", $name, $email, $phone, $dob, $gender, $profileImageName, $adminID);

            if ($stmt->execute()) {
                $success = "Personal information updated successfully.";
            } else {
                $error = "Error updating information: " . $stmt->error;
            }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['Password']) && isset($_POST['Confairm'])) {
    $password = $_POST['Password'];
    $confirmPassword = $_POST['Confairm'];
    ...
    if ($password !== $confirmPassword) {
        ...
        $updatePasswordSql = "UPDATE Admin SET password = ? WHERE adminID = ?";
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['profileImage']) && $_FILES['profileImage']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedfileExtensions)) {
            // تعيين مسار التحميل
            $uploadFileDir = 'images/icon/';
            $newFileName = 'avatar-' . time() . '.' . $fileExtension;
            $dest_path = $uploadFileDir . $newFileName;

            // نقل الملف إلى المسار المحدد
            if (move_uploaded_file($fileTmpPath, $dest_path)) {
                // الاتصال بقاعدة البيانات وتحديث مسار الصورة
                $conn = new mysqli('localhost', 'username', 'password', 'database_name');
                if ($conn->connect_error) {
                    die("Connection failed: " . $conn->connect_error);
                }

                // استبدل "1" بـ adminID المناسب للتحديث
                $adminID = 1;
                $sql = "UPDATE admin SET adminImage = ? WHERE adminID = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param('si', $dest_path, $adminID);
                $stmt->execute();
                $stmt->close();
                $conn->close();
    ...
    <div class="page-container" style="background-color: #fde4e4">
        ...
        <div class="main-content">
            <div class="section__content section__content--p30" >
                ...
                <section class="personal-info" style="margin-bottom: 100px ;background-color: #ffffff">
                                        ...
                                        <div class="row">
                                            <div class="col-md-6 col-sm-12">
                                                <div class="form-group bottom35">
                                                    ...
                                                    <input class="form-control" type="text" name="name" placeholder="Your full name" value="<?php echo htmlspecialchars($name); ?>" required="" id="registerName">
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-sm-12">
                                                <div class="form-group bottom35">
                                                    <label for="registerEmail"> Email</label>
                                                    <input class="form-control" type="email" placeholder="Your email" value="<?php echo htmlspecialchars($email); ?>" required="" name="email" id="registerEmail">
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-sm-12">
                                                <div class="form-group bottom35">
                                                    <label for="Phone">Phone</label>
                                                    <input class="form-control" type="text" placeholder="phone" value="<?php echo htmlspecialchars($phone); ?>" name="phone" required="" id="Phone">
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-sm-6">
                                                <div class="form-group bottom35">
                                                    <label for="DOB">Date Of Birth</label>
                                                    <input class="form-control" type="date" placeholder="" value="<?php echo htmlspecialchars($dob); ?>" name="DOB" required="" id="DOB">
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-sm-6">
                                                <div class="form-group bottom35">
                                                    ...
                                                    <select name="Gender" required="" class="form-control" id="Gender">
                                                        <option value="Female" <?php echo ($gender == 'Female') ? 'selected' : ''; ?> class="khaledoption">Female</option>
                                                        <option value="Male" <?php echo ($gender == 'Male') ? 'selected' : ''; ?> class="khaledoption">Male</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-sm-6">
                                                <div class="form-group bottom35">
                                                    ...
                                                    <div style="display: flex; align-items: center;">
                                                        <input type="file" class="form-control" name="profileImage" id="profileImage" accept="image/*" style="margin-right: 10px;">
                                                        <?php if (!empty($adminImage)): ?>
                                                            <img src="images/icon/<?php echo htmlspecialchars($adminImage); ?>" alt="Profile Image" style="max-width: 100px; max-height: 100px;">
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                            </div>
                ...
                <section class="personal-info" style="background-color: #ffffff">
                    <div class="container col-lg-12 col-md-12 col-sm-12 whitebox registeration-detail  wow slideInRight" data-wow-delay=".9s" style="padding: 50px 20px">
                        <div class="widget container">
                            ...
                            <form method="POST" action="" class="col-lg-12 col-md-12 col-sm-12 pr-lg-0">
                                <div class="row">
                                    <div class="col-md-6 col-sm-12">
                                        <div class="form-group bottom35">
                                    ...
                                    <div class="col-md-6 col-sm-12">
                                        <div class="form-group bottom35">
                                            <label for="Confairm">Confirmed Password</label>
                                            <input class="form-control" type="password" name="Confairm" placeholder="Confirmed Password" required="" id="Confairm">
                                        </div>
                                    </div>
                                </div>
