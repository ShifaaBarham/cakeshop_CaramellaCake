...
<head>
    ...
    <style>
        .blog__sidebar__item__categories ul li a {
            color: #000; /* اللون الافتراضي للنص */
            transition: color 0.3s ease; /* إضافة انتقال سلس */
        }

        .blog__sidebar__item__categories ul li a span {
            color: #000; /* اللون الافتراضي للرقم */
            transition: color 0.3s ease; /* إضافة انتقال سلس */
        }

        .blog__sidebar__item__categories ul li a:hover,
        .blog__sidebar__item__categories ul li a:hover span {
            color: #dd5b85; /* تغيير لون النص والرقم عند التمرير */
        }
        .blog__sidebar__recent__item__text h4 {
            color: #000; /* اللون الافتراضي للعناوين */
            transition: color 0.3s ease; /* إضافة انتقال سلس */
        }

        .blog__sidebar__recent__item:hover .blog__sidebar__recent__item__text h4 {
            color: #dd5b85; /* تغيير لون العنوان عند التمرير */
        }
        ...
        .blog__item__text a {
            color: #000; /* اللون الافتراضي للنص */
            text-decoration: none; /* إزالة أي خطوط افتراضية */
            border-bottom: 2px solid #dd5b85; /* خط سفلي زهري دائم */
            transition: color 0.3s ease, border-bottom-color 0.3s ease; /* انتقال سلس للون النص والخط */
        }
    ...
    <style>
        .blog__sidebar__recent__item {
            display: flex; /* لجعل الصورة والنص بجانب بعضهما البعض */
            align-items: center; /* لتوسيط المحتوى عموديًا */
            margin-bottom: 15px; /* مسافة بين العناصر */
        }
        .blog__sidebar__recent__item__pic img {
            width: 80px; /* حدد العرض المطلوب للصورة */
            height: 80px; /* حدد الطول المطلوب للصورة */
            object-fit: cover; /* يجعل الصورة تتناسب مع المربع */
            border-radius: 8px; /* يضيف زوايا مستديرة للصورة */
        }
...
<section class="hero" style="position: relative;background-color: #ffffff; overflow: hidden; padding-top: 1px; height:430px; margin-top: 72px; background-image: url('img/ss.jpg'); background-size: cover; background-position: center;">
    ...
    <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: rgba(255, 255, 255, 0.7); padding: 20px; border-radius: 10px; z-index: 4;">
        <h1 style="font-size: 36px; color: #f4cccc; text-align: center; font-family: 'scr' !important; text-shadow: 2px 2px 5px rgb(96,42,48);">Blog <br> Home / Blog</h1>
    </div>
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                ...
                // عدد المنشورات في كل صفحة
                $posts_per_page = 5;

                // الحصول على رقم الصفحة الحالي من URL، إذا لم يكن موجودًا، افترض الصفحة 1
                $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
                $offset = ($page - 1) * $posts_per_page;

                // استعلام لجلب عدد المنشورات الإجمالي
                $sql_total = "SELECT COUNT(*) AS total FROM post";
                $result_total = $conn->query($sql_total);
                $row_total = $result_total->fetch_assoc();
                $total_posts = $row_total['total'];

                // استعلام لجلب المنشورات مع اسم الكاتب، من الأحدث إلى الأقدم
                $sql = "
...
";
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<div class="blog__item__pic set-bg" data-setbg="img/blog/' . htmlspecialchars($row['postImage']) . '">';
                        echo '<div class="blog__pic__inner">';
                        echo '<div class="label" style="background-color:#dd5b85">Recipes</div>';
                        echo '<ul>';
                        echo '<li>By <span>' . htmlspecialchars($row['author_name']) . '</span></li>'; // اسم الكاتب
                        echo '<li>' . htmlspecialchars(date('d M Y', strtotime($row['publication_date']))) . '</li>';
                        echo '<li>' . htmlspecialchars($row['views']) . ' Views</li>';
                        echo '</ul>';
                        echo '</div>';
                        echo '</div>';
                        echo '<div class="blog__item__text">';
                        echo '<h2 style="color:#00afcc">' . htmlspecialchars($row['title']) . '</h2>';
                        echo '<p style="color:#b33461">' . htmlspecialchars(strtok($row['content'], "\n")) . '...</p>'; // عرض أول سطر من النص
                        echo '<a href="blogDetails.php?postID=' . htmlspecialchars($row['postID']) . '">READ MORE</a>';
                        echo '</div>';
                        echo '</div>';
                    }
                } else {
            ...
            <div class="col-lg-4">
                <div class="blog__sidebar">
                    ...
                    <div class="blog__sidebar__item">
                        <h5 style="color: #dd5b85;    font-family: 'Abril Fatface', serif !important;">Follow me</h5>
                    ...
                    <div class="blog__sidebar__item">
                        <h5 style="color: #dd5b85;     font-family: 'Abril Fatface', serif !important;">Popular posts</h5>
                        <?php
                        // Database connection
                        $servername = "localhost";
                        $username = "root";
                        ...
                        // SQL query to get the top 3 most viewed posts
                        $sql = "SELECT postID, title, publication_date, postImage FROM post ORDER BY views DESC LIMIT 3";
                        ...
                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo "
<a href='blogdetails.php?postID=$postID' class='blog__sidebar__recent__item'>
    <div class='blog__sidebar__recent__item__pic'>
        <img src='img/blog/$postImage' alt=''>
    </div>
    <div class='blog__sidebar__recent__item__text'>
        ...
        <span>$publication_date</span>
                    ...
                    <div class="blog__sidebar__item">
                        <h5 style="color: #dd5b85; font-family: 'Abril Fatface', serif !important;">Categories</h5>
                        <div class="blog__sidebar__item__categories">
                            <ul>
                                <li><a href="#">Recipe <span>36</span></a></li>
